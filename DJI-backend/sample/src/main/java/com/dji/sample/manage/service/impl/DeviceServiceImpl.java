...
import com.dji.sdk.config.version.DroneThingVersionEnum;
import com.dji.sdk.config.version.GatewayManager;
import com.dji.sdk.config.version.GatewayTypeEnum;
import com.dji.sdk.config.version.IThingVersion;
import com.dji.sdk.exception.CloudSDKException;
import com.dji.sdk.mqtt.*;
...
public class DeviceServiceImpl implements IDeviceService {
    ...
    public Boolean bindDevice(DeviceDTO device) {
        ...
        device.setDomain(DeviceDomainEnum.DRONE);
        device.setType(DeviceTypeEnum.M300);
        device.setSubType(DeviceSubTypeEnum.ZERO);
        device.setDeviceName("M300 Drone");


        boolean isUpd = this.saveOrUpdateDevice(device);
        if (!isUpd) {
            return false;
        }
        ...
        log.debug("Redis DeviceOPT: " + deviceOpt.toString());
        ...
        redisDevice.setWorkspaceId(deviceOpt.get().getWorkspaceId());
        redisDevice.setDeviceSn(deviceOpt.get().getDeviceSn());
        redisDevice.setParentSn(deviceOpt.get().getParentSn());
        redisDevice.setChildDeviceSn(deviceOpt.get().getChildDeviceSn());
        redisDevice.setDomain(deviceOpt.get().getDomain());
        deviceRedisService.setDeviceOnline(redisDevice);

        String sn = redisDevice.getDeviceSn();
        log.debug("Drone SN: " + sn);
        ...
        log.debug("gatewaySn: " + gatewaySn);
        log.debug("deviceSN: " + deviceSn + device.getWorkspaceId());

        pushDeviceOnlineTopo(device.getWorkspaceId(), gatewaySn, deviceSn);
        SDKManager.registerDevice(gatewaySn, deviceSn, GatewayTypeEnum.RC, null, String.valueOf(DroneThingVersionEnum.V1_0_0));
    ...
    public Optional<DeviceDTO> getDeviceBySn(String sn) {
        ...
        log.debug("getDeviceBySN: " + device + " " + deviceRedisService.checkDeviceOnline(sn));

        device.setStatus(deviceRedisService.checkDeviceOnline(sn));
        log.debug("Optional: " + Optional.of(device));
